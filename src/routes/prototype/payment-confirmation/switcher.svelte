<script lang="ts">
	...
	const label = $derived(variants.find((variant) => variant.key === current)?.name ?? current);

	function cycle(direction: 1 | -1) {
		const index = Math.max(variants.findIndex((variant) => variant.key === current), 0);
		const next = variants[(index + direction + variants.length) % variants.length];
		goto(`/prototype/payment-confirmation?variant=${next.key}`, { replaceState: true });
	}
