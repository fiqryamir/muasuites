...
> Captured: `2026-08-16T09:02:53.169Z` · via Supabase Management API (PAT) · source of truth: **live cloud, not local migrations**
...
FK: invite_id -> invites.id · package_id -> packages.id · mua_id -> muas.id
...
FK: mua_id -> muas.id · package_id -> packages.id
...
| slug | `text` | no |  |
| created_at | `timestamp with time zone` | no | `now()` |
| plan_expires_at | `timestamp with time zone` | yes |  |
...
| new_expiry | `timestamp with time zone` | yes |  |
| created_at | `timestamp with time zone` | no | `now()` |
...
## RPC functions (9) — full definitions
...
| plan_renewals | MUAs can read own renewals | public | SELECT | (auth.uid() = mua_id) |  |
...
- CREATE INDEX idx_plan_renewals_mua_created ON public.plan_renewals USING btree (mua_id, created_at DESC)
- CREATE INDEX idx_plan_renewals_pending ON public.plan_renewals USING btree (verified_at) WHERE (verified_at IS NULL)
- CREATE UNIQUE INDEX plan_renewals_pkey ON public.plan_renewals USING btree (id)
...
## REST surface (17 PostgREST paths from openapi)
...
/rpc/get_effective_plan
/rpc/cleanup_stale_bookings
```

## Size

- ~4,189 words / ~5,446 tokens (rough)
